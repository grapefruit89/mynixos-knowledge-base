# [META] ID: NIXH-CORE-003 | ADR: TBD | Version: 1.0 | Stage: 1
{
  config,
  lib,
  pkgs,
  ...
  bootSpaceCheck = pkgs.writeShellScriptBin "boot-space-check" ''
    ...
    if [ "$BOOT_USAGE" -gt 85 ]; then
      ...
      # [ADR-025] Unified Alerting (ntfy fallback to stdout if ntfy not reachable)
      curl -d "🚨 MyNixOS /boot ist zu $((BOOT_USAGE))% voll! Rebuild blockiert." http://127.0.0.1:10015/system-alerts || true
      exit 1
    fi
...
in {
  ...
  config = lib.mkIf (config.my.services.bootSafeguard.enable or true) {
    ...
    # ── MONITORING (Geist der Path Units - ADR-009) ──────────────────────────
    systemd.paths.boot-space-monitor = {
      description = "Überwacht /boot auf Dateiänderungen (ersetzt Timer)";
    ...
    systemd.services.boot-space-monitor = {
      description = "Boot Partition Space Monitor (Triggered by Path)";
      serviceConfig = {
        Type = "oneshot";
