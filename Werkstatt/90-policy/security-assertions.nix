# [META] ID: NIXH-POL-005 | ADR: TBD | Version: 1.0 | Stage: 1
{ config, lib, ... }:
let
  # 🚀 NMS v4.0 Metadaten
  nms = {
...
in
{
  ...
  config.assertions = lib.optionals (!bastelmodus) [
    ...
    # [ADR-034]: API-Key Guard (Build-Time Check)
    (must (config.sops.secrets.sonarr_api_key.path != "") "[SEC-API-001] Sonarr API-Key fehlt in SOPS!")
    (must (config.sops.secrets.radarr_api_key.path != "") "[SEC-API-002] Radarr API-Key fehlt in SOPS!")
    (must (config.sops.secrets.prowlarr_api_key.path != "") "[SEC-API-003] Prowlarr API-Key fehlt in SOPS!")
  ];
}
